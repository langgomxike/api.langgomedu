...
WHERE cvs.id = ?;`;
  ...
  public static async getUserCV3(user_id: string, onNext: (cv: any) => void) {
    await cvJoin(
      ...
      db("cvs")
        .select(
          db.raw(cvJson2("cvs", "user", "address", "gender", "icl", "im"))
        )
  ...
  public static getSugestedCVs(
    ...
    if (province || district || ward) {
      ...
      if (province) {
        condition += `ad.province LIKE?`;
        queryParams.push(`%${province}%`);
      }
      if (district) {
        condition += `OR ad.district LIKE ?`;
        queryParams.push(`%${district}%`);
      }
      if (ward) {
        condition += `OR ad.ward LIKE ?`;
